<?php
// FROM HASH: 1d816639e390c270294e3c14adc8d2f1
return array(
	...
	if ($__vars['depth'] == 1) {
		...
		<li class="p-nav-section' . (($__vars['navigation']['navigation_id'] == $__vars['selectedTab']) ? ' is-active' : '') . '" data-navigation-id="' . $__templater->escape($__vars['navigation']['navigation_id']) . '">
			<div class="p-nav-sectionHeader">
				...
				<a class="p-nav-sectionToggle js-navSectionToggle" role="button" tabindex="0" aria-label="' . $__templater->filter('Переключатель раскрытия', array(array('for_attr', array()),), true) . '"></a>
			</div>
	';
	} else if ($__vars['depth'] == 2) {
		$__finalCompiled .= '
...
<head>
	...
	<title>
		' . $__templater->func('page_title', array('%s | ' . $__vars['xf']['options']['boardTitle'] . ' - ' . 'Панель управления', $__vars['xf']['options']['boardTitle'] . ' - ' . 'Панель управления', null)) . '
	</title>
...
<header class="p-header">
	<div class="p-header-buttons p-header-buttons--main">
		<a class="p-header-button p-header-button--nav"
			...
			role="button"
			tabindex="0"
			aria-label="' . $__templater->filter('Меню', array(array('for_attr', array()),), true) . '">
			' . $__templater->fontAwesome('fa-bars', array(
	)) . '
		</a>
		<a href="' . $__templater->func('link', array('index', ), true) . '" class="p-header-button" aria-label="' . $__templater->filter('Главная', array(array('for_attr', array()),), true) . '">' . $__templater->fontAwesome('fa-home', array(
	)) . '</a>
	...
	if ($__templater->method($__vars['xf']['visitor'], 'hasAdminPermission', array('option', ))) {
		$__finalCompiled .= '
			<a class="p-header-button" data-xf-key="' . $__templater->filter('o', array(array('for_attr', array()),), true) . '" data-xf-click="menu" role="button" tabindex="0"
				aria-label="' . $__templater->filter('Настройки', array(array('for_attr', array()),), true) . '"
				aria-expanded="false"
				aria-haspopup="true">
	...
	$__finalCompiled .= '
		...
		<form action="' . $__templater->func('link', array('search/search', ), true) . '" class="menu menu--structural menu--veryWide" data-xf-init="admin-search" data-menu="menu" aria-hidden="true">
			<div class="menu-content">
				<h3 class="menu-header">' . 'Поиск' . $__vars['xf']['language']['ellipsis'] . '</h3>
				<div class="menu-row">
...
<div class="p-body-container">
	...
	if ($__vars['listenersDisabled']) {
		$__finalCompiled .= '
					<div class="blockMessage blockMessage--important blockMessage--iconic">
						' . 'Обработчики событий и расширения отключены через config.php. Это разработано как экстренная мера, позволяющая восстановить доступ к панели управления, если плагины блокируют к ней доступ. Это не должно использоваться как инструмент отладки.' . '
					</div>
				';
	}
	$__finalCompiled .= '
	...
	if ($__vars['mailDisabled']) {
		$__finalCompiled .= '
					<div class="blockMessage blockMessage--important blockMessage--iconic">
						' . 'Почта отключена в файле конфигурации. Никакие исходящие письма не будет отправляться до тех пор, пока в файле <code>config.php</code> не будет удалена строка <code>$config[\'enableMail\'] = false;</code>.' . '
					</div>
				';
	}
	$__finalCompiled .= '
...
<footer class="p-footer">
	<div class="p-footer-row">
		<div class="p-footer-row-main">
			<a href="' . $__templater->func('link', array('login/logout', null, array('t' => $__templater->func('csrf_token', array(), false), ), ), true) . '">' . 'Выйти' . '</a>
		</div>
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canChangeLanguage', array())) {
		$__finalCompiled .= '
				<a href="' . $__templater->func('link', array('account/language', ), true) . '" data-xf-click="overlay" data-xf-init="tooltip" title="' . $__templater->filter('Выбор языка', array(array('for_attr', array()),), true) . '">
