<?php
// FROM HASH: c15f26bfd201a517c9ac7d6c1b7705bb
return array(
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['phrase'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Добавить фразу');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Редактировать фразу' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['phrase']['title']));
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
';
	...
	$__finalCompiled .= '
';
	$__templater->breadcrumb($__templater->preEscaped($__templater->escape($__vars['language']['title']) . ' - ' . 'Фразы'), $__templater->func('link', array('languages/phrases', $__vars['language'], ), false), array(
	));
	$__finalCompiled .= '
...
';
	...
	if (!$__vars['language']['language_id']) {
		$__compilerTemp4 .= '
			...
			'label' => 'Кэшировать эту фразу глобально',
			'_type' => 'option',
		)), array(
		)) . '
			';
	...
	<div class="block-container">
		...
		'label' => 'Текст фразы',
		'explain' => 'Вы можете использовать здесь HTML, но лучше и правильнее делать разметку в шаблонах, а не во фразах.',
	)) . '
