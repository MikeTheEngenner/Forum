...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['definitionEditable']) {
		$__compilerTemp3 .= '
			<a href="' . $__templater->func('link', array('style-properties/edit', $__vars['property'], ), true) . '">' . 'Изменить' . '</a>
		';
	}
	$__compilerTemp4 = '';
	...
	';
		...
		if ($__vars['valueOptions']['template']) {
			...
			if ($__templater->test($__vars['includeHtml'], 'empty', array())) {
				$__finalCompiled .= '
				' . $__templater->formRow('
					<div class="blockMessage blockMessage--error blockMessage--iconic">' . 'Шаблон ' . $__templater->escape($__vars['valueOptions']['template']) . ' не найден.' . '</div>
				', array(
					'label' => $__templater->escape($__vars['titleHtml']),
					'hint' => $__templater->escape($__vars['hintHtml']),
				)) . '
		...
		';
			...
			$__finalCompiled .= '
			' . $__templater->formRow('
				<div class="blockMessage blockMessage--error blockMessage--iconic">' . 'Шаблон не указан.' . '</div>
			', array(
				'label' => $__templater->escape($__vars['titleHtml']),
				'hint' => $__templater->escape($__vars['hintHtml']),
			)) . '
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['customizationState'] == 'custom') {
		$__compilerTemp1 .= '
							<span class="cssPropertyRevert" data-xf-init="tooltip" title="' . $__templater->filter('Отменить изменения, сделанные в этом стиле', array(array('for_attr', array()),), true) . '">
								' . $__templater->callMacro(null, 'revert_code', array(
			'submitName' => $__vars['submitName'],
			'property' => $__vars['property'],
			'label' => 'Вернуть значение по умолчанию',
			'container' => '< .block-row',
		), $__vars) . '
							</span>
						';
	...
	if ($__templater->method($__vars['property'], 'isValidCssComponent', array('text', ))) {
		$__finalCompiled .= '
						<li>
							<h4 class="cssPropertyHeader">' . 'Текст' . '</h4>
		...
		array(
			...
			'label' => '
											' . 'Не использовать оформление текста' . '
										',
			'_type' => 'option',
		))) . '
								</td>
	...
	if ($__templater->method($__vars['property'], 'isValidCssComponent', array('background', ))) {
		$__finalCompiled .= '
						<li>
							<h4 class="cssPropertyHeader">' . 'Фон' . '</h4>
							<table class="cssPropertySet">
							<tr class="cssPropertySet-headerRow">
								<th>' . 'Цвет' . '</th>
							</tr>
	...
	if ($__templater->method($__vars['property'], 'isValidCssComponent', array('border', )) OR $__templater->method($__vars['property'], 'isValidCssComponent', array('border_radius', ))) {
		$__finalCompiled .= '
						<li>
							...
							<tr class="cssPropertySet-headerRow cssPropertySet-headerRow--separated">
								<th class="cssPropertySet-headerSetLabel">' . 'Рамка' . '</th>
								';
		...
		$__finalCompiled .= '
							...
							<tr>
								<td class="cssPropertySet-rowLabel">' . 'Все' . '</td>
		...
		$__finalCompiled .= '
							...
							<tr>
								<td class="cssPropertySet-rowLabel">' . 'Верх' . '</td>
		...
		$__finalCompiled .= '
							...
							<tr>
								<td class="cssPropertySet-rowLabel">' . 'Справа' . '</td>
		...
		$__finalCompiled .= '
							...
							<tr>
								<td class="cssPropertySet-rowLabel">' . 'Низ' . '</td>
		...
		$__finalCompiled .= '
							...
							<tr>
								<td class="cssPropertySet-rowLabel">' . 'Слева' . '</td>
		...
		$__finalCompiled .= '
						<li>
							<h4 class="cssPropertyHeader">' . 'Рамка' . '</h4>
							<table class="cssPropertySet">
							';
	...
	if ($__templater->method($__vars['property'], 'isValidCssComponent', array('padding', ))) {
		$__finalCompiled .= '
						<li>
							<h4 class="cssPropertyHeader">' . 'Внутренний отступ' . '</h4>
							<table class="cssPropertySet">
							<tr>
								<td class="cssPropertySet-rowLabel">' . 'Все' . '</td>
	...
	if ($__templater->method($__vars['property'], 'isValidCssComponent', array('extra', ))) {
		$__finalCompiled .= '
						<li class="cssPropertyExtra">
							<h4 class="cssPropertyHeader">' . 'Дополнительно' . '</h4>
							<table class="cssPropertySet">
								<tr class="cssPropertySet-headerRow">
									<th>' . 'Произвольный CSS/LESS код' . '</th>
								</tr>
								<tr>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['state'] == 'custom') {
		$__compilerTemp1 .= '
				...
				<span data-xf-init="tooltip" title="' . $__templater->filter('Значение этого свойства было переопределено в этом стиле', array(array('for_attr', array()),), true) . '">
					';
		if ($__vars['checkbox']) {
			$__compilerTemp1 .= '
						' . $__templater->callMacro(null, 'revert_code', array(
		...
		$__compilerTemp1 .= '
				...
				<span  data-xf-init="tooltip" title="' . $__templater->filter('Изменённое значение для этого свойства наследуется из родительского стиля', array(array('for_attr', array()),), true) . '">
					' . 'Значение унаследовано' . '</span>

			';
	} else if ($__vars['state'] == 'added') {
		$__compilerTemp1 .= '

				<span data-xf-init="tooltip" title="' . $__templater->filter('Это свойство было создано в этом стиле и не существует в родительских', array(array('for_attr', array()),), true) . '">
					' . 'Пользовательское свойство' . '</span>
