<?php
// FROM HASH: d12e21010ccb194df3e007875d18a5f1
return array(
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['template'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Добавить шаблон');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Редактировать шаблон' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['template']['title']));
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
';
	...
	$__finalCompiled .= '
';
	$__templater->breadcrumb($__templater->preEscaped($__templater->escape($__vars['style']['title']) . ' - ' . 'Шаблоны'), $__templater->func('link', array('styles/templates', $__vars['style'], array('type' => $__vars['template']['type'], ), ), false), array(
	));
	$__finalCompiled .= '
...
';
	if ($__templater->method($__vars['template'], 'isUpdate', array()) AND ($__vars['template']['style_id'] == $__vars['style']['style_id'])) {
		...
		if ($__vars['style']['style_id']) {
			$__compilerTemp1 .= '
		' . $__templater->button('Посмотреть пользовательские изменения', array(
				'href' => $__templater->func('link', array('templates/compare', $__vars['template'], ), false),
				'data-xf-click' => 'overlay',
				'data-cache' => 'false',
			), '', array(
			)) . '
		' . $__templater->button('Вернуть', array(
...
';
	...
	if ($__vars['hasHistory']) {
		$__compilerTemp4 .= '
					' . $__templater->button('Посмотреть историю', array(
	...
	<div class="block-outer">
		...
		<div class="block-outer-opposite"><span class="block-outer-hint">' . 'Тип шаблона' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['types'][$__vars['template']['type']]) . '</span></div>
	</div>
	...
	<div class="block-container">
		...
		'label' => 'Шаблон',
		'explain' => 'Здесь Вы можете использовать специальный синтаксис шаблонов XenForo.',
		'finalhtml' => '
					' . $__compilerTemp2 . '
				',
	)) . '
