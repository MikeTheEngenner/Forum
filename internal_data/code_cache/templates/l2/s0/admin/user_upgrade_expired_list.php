...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Истёкшие повышения прав' . ($__vars['userUpgrade'] ? (': ' . $__templater->escape($__vars['userUpgrade']['title'])) : ''));
	$__finalCompiled .= '
...
';
	...
	<div class="block">
		...
		$__finalCompiled .= $__templater->dataList('
			...
			'_type' => 'cell',
			'html' => '<a href="' . $__templater->func('link', array('user-upgrades/expired', $__vars['userUpgrade'], array('order' => 'username', 'direction' => '', ) + $__vars['linkParams'], ), true) . '">' . 'Пользователь' . '</a>',
		),
		array(
			'_type' => 'cell',
		...
		array(
			'_type' => 'cell',
			'html' => 'Платёжный профиль',
		),
		array(
			'_type' => 'cell',
			'html' => '<a href="' . $__templater->func('link', array('user-upgrades/expired', $__vars['userUpgrade'], array('order' => 'start_date', 'direction' => 'desc', ) + $__vars['linkParams'], ), true) . '">' . 'Дата начала' . '</a>',
		),
		array(
			'_type' => 'cell',
			'html' => '<a href="' . $__templater->func('link', array('user-upgrades/expired', $__vars['userUpgrade'], array('order' => 'end_date', 'direction' => 'desc', ) + $__vars['linkParams'], ), true) . '">' . 'Дата окончания' . '</a>',
		))) . '
					' . $__compilerTemp1 . '
				', array(
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Результатов не найдено.' . '</div>
';
	}
