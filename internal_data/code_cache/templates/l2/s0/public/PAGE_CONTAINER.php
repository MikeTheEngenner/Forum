<?php
// FROM HASH: dd8c7d16c5353fe69b843e5e7e28ee29
return array(
	...
	if ($__vars['nav']['href']) {
		...
		if ($__vars['nav']['children']) {
			$__finalCompiled .= '<a data-xf-key="' . $__templater->escape($__vars['shortcut']) . '"
				...
				role="button"
				tabindex="0"
				aria-label="' . $__templater->filter('Переключатель раскрытия', array(array('for_attr', array()),), true) . '"
				aria-expanded="false"
...
';
	...
	if (strlen(trim($__compilerTemp2)) > 0) {
		$__compilerTemp1 .= '
					<a class="p-staffBar-link menuTrigger" data-xf-click="menu" data-xf-key="alt+m" role="button" tabindex="0" aria-expanded="false" aria-haspopup="true">' . 'Модератор' . '</a>
			...
			$__compilerTemp4 .= '
				...
				'title' => '',
							...
							<a href="' . $__templater->func('link', array('logout', null, array('t' => $__templater->func('csrf_token', array(), false), ), ), true) . '" class="p-navgroup-link">
								<span class="p-navgroup-linkText">' . 'Выйти' . '</span>
							</a>
						';
		} else {
			$__compilerTemp4 .= '
				...
				'title' => '',
							...
							<a href="' . $__templater->func('link', array('conversations', ), true) . '"
								...
								aria-label="' . $__templater->filter('Переписки', array(array('for_attr', array()),), true) . '"
								aria-expanded="false"
								aria-haspopup="true">
									...
									<div class="menu-footer menu-footer--split">
										<div class="menu-footer-main">
											<ul class="listInline listInline--bullet">
												...
												<li><a href="' . $__templater->func('link', array('account/alerts/mark-read', ), true) . '" class="js-alertsMarkRead">' . 'Отметить прочитанным' . '</a></li>
		...
		if ($__vars['xf']['options']['enableTagging']) {
			$__compilerTemp7 .= '
													<span tabindex="0" role="button"
														data-xf-init="tooltip" data-trigger="hover focus click" title="' . $__templater->filter('Теги также будут учитываться', array(array('for_attr', array()),), true) . '">
			...
			'_type' => 'option',
								...
								<div class="menu-row">
									<div class="inputGroup">
										<span class="inputGroup-text" id="ctrl_search_menu_by_member">' . 'Автор' . $__vars['xf']['language']['label_separator'] . '</span>
...
';
	...
	<div class="u-scrollButtons js-scrollButtons" data-trigger-type="' . $__templater->func('property', array('scrollJumpButtons', ), true) . '">
		...
		)) . '<span class="u-srOnly">' . 'Верх' . '</span>', array(
			'href' => '#top',
			'class' => 'button--scroll',
		...
		if ($__templater->func('property', array('scrollJumpButtons', ), false) != 'up') {
			...
			)) . '<span class="u-srOnly">' . 'Низ' . '</span>', array(
				'href' => '#footer',
				'class' => 'button--scroll',
				'data-xf-click' => 'scroll-to',
			), '', array(
