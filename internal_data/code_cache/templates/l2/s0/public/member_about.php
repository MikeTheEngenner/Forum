<?php
// FROM HASH: 2b14b9a971532af0568135dd6945370c
return array(
...
<div class="block">
	...
	if ($__vars['user']['Profile']['birthday']['timeStamp']) {
		...
		if ($__vars['user']['Profile']['birthday']['age']) {
			$__compilerTemp3 .= '
											' . $__templater->func('parens', array(('Возраст' . $__vars['xf']['language']['label_separator'] . ' ') . $__vars['user']['Profile']['birthday']['age'], ), true) . '
										';
		}
		$__compilerTemp3 .= '
									';
	...
	if ($__templater->method($__vars['user'], 'canViewIdentities', array())) {
		...
		if ($__templater->method($__vars['xf']['visitor'], 'canStartConversationWith', array($__vars['user'], ))) {
			$__compilerTemp4 .= '
								<dl class="pairs pairs--columns pairs--fixedSmall">
									<dt>' . 'Переписка' . '</dt>
									<dd><a href="' . $__templater->func('link', array('conversations/add', '', array('to' => $__vars['user']['username'], ), ), true) . '">' . 'Начать переписку' . '</a></dd>
								</dl>
							';
		}
		$__compilerTemp4 .= '
		...
		if (strlen(trim($__compilerTemp4)) > 0) {
			$__compilerTemp1 .= '
					<div class="block-row block-row--separated">
						<h4 class="block-textHeader">' . 'Связь' . '</h4>
						' . $__compilerTemp4 . '
					</div>
				';
		}
	...
	if ($__vars['followingCount']) {
		...
		if (strlen(trim($__compilerTemp5)) > 0) {
			...
			if ($__vars['followingCount'] > $__templater->func('count', array($__vars['following'], ), false)) {
				$__compilerTemp1 .= '
							<a href="' . $__templater->func('link', array('members/following', $__vars['user'], ), true) . '" data-xf-click="overlay">' . '...и ещё ' . ($__vars['followingCount'] - $__templater->func('count', array($__vars['following'], ), false)) . '.' . '</a>
						';
			}
			$__compilerTemp1 .= '
					</div>
	...
	if ($__vars['followersCount']) {
		...
		if (strlen(trim($__compilerTemp6)) > 0) {
			...
			if ($__vars['followersCount'] > $__templater->func('count', array($__vars['followers'], ), false)) {
				$__compilerTemp1 .= '
							<a href="' . $__templater->func('link', array('members/followers', $__vars['user'], ), true) . '" data-xf-click="overlay">' . '...и ещё ' . ($__vars['followersCount'] - $__templater->func('count', array($__vars['followers'], ), false)) . '.' . '</a>
						';
			}
			$__compilerTemp1 .= '
					</div>
	...
	if (strlen(trim($__compilerTemp1)) > 0) {
		...
		$__finalCompiled .= '
			<div class="block-row block-row--separated">' . '' . $__templater->escape($__vars['user']['username']) . ' не предоставил никакой дополнительной информации.' . '</div>
		';
	}
	$__finalCompiled .= '
		</div>
