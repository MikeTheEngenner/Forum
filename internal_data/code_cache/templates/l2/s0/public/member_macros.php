...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canCleanSpam', array()) AND $__templater->method($__vars['user'], 'isPossibleSpammer', array())) {
		$__finalCompiled .= '
		<a href="' . $__templater->func('link', array('spam-cleaner', $__vars['user'], array('no_redirect' => 1, ), ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Спам' . '</a>
	';
	}
	$__finalCompiled .= '
	';
	if ($__templater->method($__vars['user'], 'canWarn', array())) {
		$__finalCompiled .= '
		<a href="' . $__templater->func('link', array('members/warn', $__vars['user'], ), true) . '" class="menu-linkRow">' . 'Предупредить' . '</a>
	';
	}
	$__finalCompiled .= '
	';
	if ($__templater->method($__vars['xf']['visitor'], 'canViewWarnings', array()) AND ($__vars['user']['warning_count'] AND ($__vars['context'] == 'tooltip'))) {
		$__finalCompiled .= '
		<a href="' . ($__templater->method($__vars['user'], 'canViewFullProfile', array()) ? ($__templater->func('link', array('members', $__vars['user'], ), true) . '#warnings') : $__templater->func('link', array('members/warnings', $__vars['user'], ), true)) . '" class="menu-linkRow">' . 'Посмотреть предупреждения (' . $__templater->filter($__vars['user']['warning_count'], array(array('number', array()),), true) . ')' . '</a>
	';
	}
	$__finalCompiled .= '
	';
	if ($__templater->method($__vars['xf']['visitor'], 'canViewIps', array())) {
		$__finalCompiled .= '
		<a href="' . $__templater->func('link', array('members/user-ips', $__vars['user'], ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'IP-адреса' . '</a>
		<a href="' . $__templater->func('link', array('members/shared-ips', $__vars['user'], ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Общие IP-адреса' . '</a>
	';
	}
	$__finalCompiled .= '
	';
	...
	if ($__templater->method($__vars['user'], 'canEdit', array())) {
		$__finalCompiled .= '
		<a href="' . $__templater->func('link', array('members/edit', $__vars['user'], ), true) . '" class="menu-linkRow">' . 'Изменить' . '</a>
	';
	}
	$__finalCompiled .= '
	' . '
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['xf']['options']['enableTrophies']) {
		$__finalCompiled .= '
		<dl class="pairs pairs--rows pairs--rows--centered fauxBlockLink">
			<dt title="' . $__templater->filter('Баллы', array(array('for_attr', array()),), true) . '">' . 'Баллы' . '</dt>
			<dd>
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canIgnoreUser', array($__vars['user'], ))) {
		$__compilerTemp1 .= '
				<a href="' . $__templater->func('link', array('members/ignore', $__vars['user'], ), true) . '"
					...
					data-xf-click="switch"
					data-sk-ignore="' . $__templater->filter('+ В игнор', array(array('for_attr', array()),), true) . '"
					data-sk-unignore="' . $__templater->filter('- Из игнора', array(array('for_attr', array()),), true) . '">
					' . ($__templater->method($__vars['xf']['visitor'], 'isIgnoring', array($__vars['user'], )) ? '- Из игнора' : '+ В игнор') . '
				</a>
			';
	}
	$__compilerTemp1 .= '
	...
	if ($__templater->method($__vars['xf']['visitor'], 'canSearch', array()) AND ($__vars['context'] != 'tooltip')) {
		$__compilerTemp2 .= '
				<div class="buttonGroup-buttonWrapper">
					' . $__templater->button('Найти', array(
			...
			'aria-haspopup' => 'true',
					...
					<div class="menu" data-menu="menu" aria-hidden="true">
						<div class="menu-content">
							...
							' . '
							<a href="' . $__templater->func('link', array('search/member', null, array('user_id' => $__vars['user']['user_id'], ), ), true) . '" rel="nofollow" class="menu-linkRow">' . 'Найти весь контент от ' . $__templater->escape($__vars['user']['username']) . '' . '</a>
							<a href="' . $__templater->func('link', array('search/member', null, array('user_id' => $__vars['user']['user_id'], 'content' => 'thread', ), ), true) . '" rel="nofollow" class="menu-linkRow">' . 'Найти все темы от ' . $__templater->escape($__vars['user']['username']) . '' . '</a>
							' . '
						</div>
					</div>
				</div>
