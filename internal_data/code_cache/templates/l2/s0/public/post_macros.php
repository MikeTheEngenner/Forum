<?php
// FROM HASH: c62fd032c659f6ad40261e3413c5915c
return array(
...
'arguments' => function($__templater, array $__vars) { return array(
		...
		'repliesPhrase' => 'Ответы' . $__vars['xf']['language']['label_separator'],
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['showThreadStateIcons']) {
		...
		if ($__vars['xf']['visitor']['user_id'] AND $__vars['thread']['Watch'][$__vars['xf']['visitor']['user_id']]) {
			$__finalCompiled .= '
				...
				'title' => $__templater->filter('Отслеживаемая тема', array(array('for_attr', array()),), false),
			)) . '
		...
		if ($__vars['thread']['sticky']) {
			$__finalCompiled .= '
				...
				'title' => $__templater->filter('Закреплено', array(array('for_attr', array()),), false),
			)) . '
	...
	if ($__vars['showUser']) {
		...
		$__finalCompiled .= '
				<li class="u-concealed message-attribution-user--top">' . 'Автор ' . $__templater->escape($__vars['username']) . '' . '</li>
			';
	}
	$__finalCompiled .= '
	...
	if ($__templater->method($__vars['post'], 'isUnread', array())) {
		$__finalCompiled .= '
				<li><span class="message-newIndicator">' . 'Новое' . '</span></li>
			';
	} else if ($__templater->method($__vars['thread'], 'isUnread', array()) AND $__vars['showThreadUnreadIndicator']) {
		$__finalCompiled .= '
				<li><span class="message-newIndicator" title="' . $__templater->filter('Новые ответы', array(array('for_attr', array()),), true) . '">' . 'Новое' . '</span></li>
			';
	}
	$__finalCompiled .= '
			<li>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['post']['last_edit_date']) {
		...
		if ($__vars['post']['user_id'] == $__vars['post']['last_edit_user_id']) {
			$__finalCompiled .= '
				' . 'Последнее редактирование' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['post']['last_edit_date'], array(
				'itemprop' => 'dateModified',
			))) . '
			';
		} else {
			$__finalCompiled .= '
				' . 'Последнее редактирование модератором' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['post']['last_edit_date'], array(
				'itemprop' => 'dateModified',
			))) . '
			';
		}
	...
	if ($__templater->method($__vars['thread'], 'canReply', array()) OR $__templater->method($__vars['thread'], 'canReplyPreReg', array())) {
		...
		$__compilerTemp1 .= '
				...
				<a href="' . $__templater->escape($__vars['quoteLink']) . '"
					...
					data-xf-click="quote"
					data-quote-href="' . $__templater->func('link', array('posts/quote', $__vars['post'], ), true) . '">' . 'Ответ' . '</a>
			';
	}
	$__compilerTemp1 .= '
		';
	...
	if ($__vars['post']['edit_count'] AND $__templater->method($__vars['post'], 'canViewHistory', array())) {
		$__compilerTemp2 .= '
				<a href="' . $__templater->func('link', array('posts/history', $__vars['post'], ), true) . '"
					...
					data-menu-closer="true">' . 'История версий' . '</a>
	...
	if (($__vars['post']['message_state'] == 'deleted') AND $__templater->method($__vars['post'], 'canUndelete', array())) {
		$__compilerTemp2 .= '
				<a href="' . $__templater->func('link', array('posts/undelete', $__vars['post'], ), true) . '"
					...
					data-xf-click="overlay">' . 'Восстановить' . '</a>
...
';
	...
	<div class="message message--deleted message--post' . ($__templater->method($__vars['post'], 'isIgnored', array()) ? ' is-ignored' : '') . ($__templater->method($__vars['post'], 'isUnread', array()) ? ' is-unread' : '') . ' js-post js-inlineModContainer"
		...
		'log' => $__vars['post']['DeletionLog'],
					...
					<a href="' . $__templater->func('link', array('posts/show', $__vars['post'], ), true) . '" class="u-jsOnly" data-xf-click="inserter" data-replace="[data-content=post-' . $__templater->escape($__vars['post']['post_id']) . ']">' . 'Показать' . $__vars['xf']['language']['ellipsis'] . '</a>
