...
'extensions' => array('icon_cell' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['thread'], 'getUserPostCount', array())) {
		$__finalCompiled .= '
			...
			'data-trigger' => 'auto',
			'title' => 'Ваших сообщений в этой теме: ' . $__templater->method($__vars['thread'], 'getUserPostCount', array()) . '',
		))) . '
				';
	}
	$__finalCompiled .= '
...
'status_sticky' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['thread']['sticky']) {
		$__finalCompiled .= '
							<li>
								<i class="structItem-status structItem-status--sticky" aria-hidden="true" title="' . $__templater->filter('Закреплено', array(array('for_attr', array()),), true) . '"></i>
...
'thread_type_redirect' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
		$__finalCompiled .= '
							<li>
								<i class="structItem-status structItem-status--redirect" aria-hidden="true" title="' . $__templater->filter('Переадресация', array(array('for_attr', array()),), true) . '"></i>
...
'thread_type_question_solved' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
		$__finalCompiled .= '
							<li>
								<i class="structItem-status structItem-status--solved" aria-hidden="true" title="' . $__templater->filter('Решён', array(array('for_attr', array()),), true) . '"></i>
								<span class="u-srOnly">' . 'Решён' . '</span>
							</li>
						';
	return $__finalCompiled;
},
...
'statuses' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['thread']['discussion_state'] == 'moderated') {
		$__finalCompiled .= '
						<li>
							<i class="structItem-status structItem-status--moderated" aria-hidden="true" title="' . $__templater->filter('Ожидает одобрения', array(array('for_attr', array()),), true) . '"></i>
							<span class="u-srOnly">' . 'Ожидает одобрения' . '</span>
						</li>
					';
	}
	$__finalCompiled .= '
	...
	if ($__vars['thread']['discussion_state'] == 'deleted') {
		$__finalCompiled .= '
						<li>
							<i class="structItem-status structItem-status--deleted" aria-hidden="true" title="' . $__templater->filter('Удалено', array(array('for_attr', array()),), true) . '"></i>
							<span class="u-srOnly">' . 'Удалено' . '</span>
						</li>
					';
	}
	$__finalCompiled .= '
	...
	if (!$__vars['thread']['discussion_open']) {
		$__finalCompiled .= '
						<li>
							<i class="structItem-status structItem-status--locked" aria-hidden="true" title="' . $__templater->filter('Закрыто', array(array('for_attr', array()),), true) . '"></i>
	...
	if ($__vars['showWatched'] AND $__vars['xf']['visitor']['user_id']) {
		...
		if ($__vars['thread']['Watch'][$__vars['xf']['visitor']['user_id']]) {
			$__finalCompiled .= '
							<li>
								<i class="structItem-status structItem-status--watched" aria-hidden="true" title="' . $__templater->filter('Отслеживаемая тема', array(array('for_attr', array()),), true) . '"></i>
			...
			$__finalCompiled .= '
							<li>
								<i class="structItem-status structItem-status--watched" aria-hidden="true" title="' . $__templater->filter('Отслеживаемый раздел', array(array('for_attr', array()),), true) . '"></i>
								<span class="u-srOnly">' . 'Отслеживаемый раздел' . '</span>
							</li>
						';
		}
		$__finalCompiled .= '
...
'meta_cell' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
		$__finalCompiled .= '
		<div class="structItem-cell structItem-cell--meta" title="' . $__templater->filter('Количество реакций на первое сообщение' . $__vars['xf']['language']['label_separator'], array(array('for_attr', array()),), true) . ' ' . $__templater->filter($__vars['thread']['first_post_reaction_score'], array(array('number', array()),), true) . '">
	...
	$__finalCompiled .= '
			...
			<div class="contentRow-minor contentRow-minor--hideLinks">
				<ul class="listInline listInline--bullet">
					<li>' . 'Последнее: ' . $__templater->escape($__vars['thread']['last_post_cache']['username']) . '' . '</li>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<div class="contentRow">
		...
		<div class="contentRow-main contentRow-main--close">
			...
			<div class="contentRow-minor contentRow-minor--hideLinks">
				<ul class="listInline listInline--bullet">
					<li>' . 'Автор ' . $__templater->escape($__vars['thread']['username']) . '' . '</li>
					<li>' . $__templater->func('date_dynamic', array($__vars['thread']['post_date'], array(
	))) . '</li>
					<li>' . 'Ответы' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['thread']['reply_count'], array(array('number_short', array()),), true) . '</li>
				</ul>
			</div>
			<div class="contentRow-minor contentRow-minor--hideLinks">
				<a href="' . $__templater->func('link', array('forums', $__vars['thread']['Forum'], ), true) . '">' . $__templater->escape($__vars['thread']['Forum']['title']) . '</a>
...
'main_cell' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
		...
		'title' => 'Создать новую тему в этом разделе',
		...
		'label' => 'Заголовок',
				...
				<div class="js-quickThreadFields inserter-container is-hidden"><!--' . 'Загрузка' . $__vars['xf']['language']['ellipsis'] . '--></div>
			</div>
		';
	return $__finalCompiled;
}),
