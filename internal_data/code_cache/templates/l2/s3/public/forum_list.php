...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if (strlen(trim($__compilerTemp1)) > 0) {
		...
		<div class="node-subNodesFlat">
			<span class="node-subNodesLabel">' . 'Подразделы' . $__vars['xf']['language']['label_separator'] . '</span>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if (($__vars['xf']['options']['forumsDefaultPage'] != 'forums') OR ($__vars['xf']['options']['indexRoute'] AND ($__vars['xf']['options']['indexRoute'] != 'forums/'))) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Список разделов');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
...
';
	$__templater->setPageParam('head.' . 'rss_forum', $__templater->preEscaped('<link rel="alternate" type="application/rss+xml" title="' . $__templater->filter('RSS лента для ' . $__vars['xf']['options']['boardTitle'] . '', array(array('for_attr', array()),), true) . '" href="' . $__templater->func('link', array('forums/index.rss', '-', ), true) . '" />'));
	$__finalCompiled .= '
