<?php
// FROM HASH: e7a8817508feeb9ddac50fb557c187f6
return array(
...
<div class="block">
	...
	if (($__vars['user']['user_id'] == $__vars['xf']['visitor']['user_id']) AND $__templater->method($__vars['xf']['visitor'], 'canUploadAvatar', array())) {
		$__compilerTemp1 .= '
									<a class="avatarWrapper-update" href="' . $__templater->func('link', array('account/avatar', ), true) . '" data-xf-click="overlay"><span>' . 'Изменить' . '</span></a>
								';
	}
	$__compilerTemp2 = '';
	$__compilerTemp3 = '';
	...
	if (strlen(trim($__compilerTemp5)) > 0) {
		$__compilerTemp4 .= '
												<div class="buttonGroup-buttonWrapper">
													' . $__templater->button('Инструменты модератора', array(
			...
			'aria-haspopup' => 'true',
													...
													<div class="menu" data-menu="menu" aria-hidden="true">
														<div class="menu-content">
															<h3 class="menu-header">' . 'Инструменты модератора' . '</h3>
															' . $__compilerTemp5 . '
														</div>
													</div>
												</div>
	...
	if ($__templater->method($__vars['user'], 'hasViewableUsernameHistory', array())) {
		$__compilerTemp6 .= '
								<a class="memberHeader-nameChangeIndicator"
									...
									title="' . $__templater->filter('Имя пользователя изменено', array(array('for_attr', array()),), true) . '"
									role="button"
									tabindex="0"
									aria-expanded="false"
									aria-haspopup="true"
									aria-label="' . $__templater->filter('Имя пользователя изменено', array(array('for_attr', array()),), true) . '">' . $__templater->fontAwesome('fa-history', array(
		)) . '</a>
								<div class="menu" data-menu="menu" aria-hidden="true"
...
<ul class="tabPanes js-memberTabPanes">
	...
	if ($__templater->method($__vars['user'], 'canViewPostsOnProfile', array())) {
		...
		if (!$__templater->test($__vars['profilePosts'], 'empty', array())) {
			...
			$__finalCompiled .= '
							<div class="block-row js-replyNoMessages">' . 'В профиле ' . $__templater->escape($__vars['user']['username']) . ' пока нет ни одного сообщения.' . '</div>
						';
		}
		$__finalCompiled .= '
					</div>
	...
	if ($__templater->method($__vars['user'], 'canViewLatestActivity', array())) {
		...
		<li data-href="' . $__templater->func('link', array('members/latest-activity', $__vars['user'], ), true) . '" role="tabpanel" aria-labelledby="latest-activity">
			<div class="blockMessage">' . 'Загрузка' . $__vars['xf']['language']['ellipsis'] . '</div>
		</li>
	';
	}
	$__finalCompiled .= '
