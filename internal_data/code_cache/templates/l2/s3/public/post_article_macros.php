<?php
// FROM HASH: 60d1ad2b5f4e1f278315fa9a3395feba
return array(
	...
	if ($__vars['isListItem']) {
		...
		if ($__vars['thread']['discussion_state'] == 'moderated') {
			$__finalCompiled .= '
				...
				'aria-hidden' => 'true',
				'title' => $__templater->filter('Ожидает одобрения', array(array('for_attr', array()),), false),
			)) . '
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['thread']['reply_count']) {
		$__compilerTemp1 .= '
			...
			'class' => 'u-spaceAfter',
		)) . 'Ответы' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['thread']['reply_count'], array(array('number', array()),), true) . '</a>
						';
	}
	$__compilerTemp1 .= '
						';
...
'extensions' => array('status_new' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['thread'], 'isUnread', array())) {
		$__finalCompiled .= '
								<li><span class="message-newIndicator" title="' . ((!$__templater->method($__vars['post'], 'isUnread', array())) ? 'Новые ответы' : '') . '">' . 'Новое' . '</span></li>
							';
	}
	$__finalCompiled .= '
						';
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['thread']['cover_image']) {
		$__finalCompiled .= '
				<a href="' . $__templater->escape($__vars['linkUnread']) . '" class="articlePreview-image" style="background-image: url(\'' . $__templater->escape($__vars['thread']['cover_image']) . '\')">&nbsp;</a>
			';
	}
	$__finalCompiled .= '
	...
	$__finalCompiled .= '
				...
				<div class="articlePreview-links">
					...
					<a href="' . $__templater->escape($__vars['link']) . '">' . 'Посмотреть всю статью' . ' &gt;</a>
				</div>
			</div>
		</div>
		...
		'defaultname' => $__vars['thread']['username'],
				...
				<li class="articlePreview-by">' . 'Автор ' . $__templater->escape($__vars['username']) . '' . '</li>
	...
	if ($__vars['thread']['reply_count']) {
		$__finalCompiled .= '
					<li class="articlePreview-replies"><a href="' . $__templater->escape($__vars['linkUnread']) . '"
						title="' . 'Ответы' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['thread']['reply_count'], array(array('number', array()),), true) . '">
						' . $__templater->fontAwesome('fa-comment', array(
			'class' => 'u-spaceAfter',
		)) . '<span class="articlePreview-repliesLabel">' . 'Ответы' . $__vars['xf']['language']['label_separator'] . ' </span>' . $__templater->filter($__vars['thread']['reply_count'], array(array('number', array()),), true) . '</a></li>
				';
	}
	$__finalCompiled .= '
				';
