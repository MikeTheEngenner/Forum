<?php
// FROM HASH: f1c6b9c43db7d0eb1106a5acad4ecb77
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['showTargetUser'] AND ($__vars['profilePost']['user_id'] != $__vars['profilePost']['profile_user_id'])) {
		...
		<span class="u-srOnly">' . '' . ($__templater->escape($__vars['profilePost']['User']['username']) ?: $__templater->escape($__vars['profilePost']['username'])) . ' написал в профиле ' . ($__templater->escape($__vars['profilePost']['ProfileUser']['username']) ?: 'Неизвестно') . '.' . '</span>
	';
	} else {
		$__finalCompiled .= '
		' . $__templater->func('username_link', array($__vars['profilePost']['User'], true, array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['profilePost']['message_state'] == 'deleted') {
		...
		$__finalCompiled .= '
							<div class="messageNotice messageNotice--moderated">
								' . 'Это сообщение ожидает одобрения модератора и скрыто от обычных посетителей.' . '
							</div>
						';
	}
	$__finalCompiled .= '
	...
	if ($__templater->method($__vars['profilePost'], 'isIgnored', array())) {
		$__finalCompiled .= '
							<div class="messageNotice messageNotice--ignored">
								' . 'Вы игнорируете контент этого пользователя.' . '
							</div>
						';
	}
	$__finalCompiled .= '
	...
	if ($__vars['allowInlineMod'] AND $__templater->method($__vars['profilePost'], 'canUseInlineModeration', array())) {
		$__compilerTemp2 .= '
			...
			'title' => 'Выбрать для модерации',
			'label' => 'Выбрать для модерации',
			'hiddenlabel' => 'true',
			'_type' => 'option',
		))) . '
										</span>
	...
	if ($__templater->method($__vars['profilePost'], 'canReport', array())) {
		$__compilerTemp2 .= '
										<a href="' . $__templater->func('link', array('profile-posts/report', $__vars['profilePost'], ), true) . '" class="actionBar-action actionBar-action--report" data-xf-click="overlay">' . 'Жалоба' . '</a>
									';
	}
	$__compilerTemp2 .= '
	...
	if ($__templater->method($__vars['profilePost'], 'canEdit', array())) {
		...
		$__compilerTemp2 .= '
										<a href="' . $__templater->func('link', array('profile-posts/edit', $__vars['profilePost'], ), true) . '"
											...
											data-menu-closer="true">' . 'Изменить' . '</a>
										';
	...
	if ($__templater->method($__vars['profilePost'], 'canDelete', array('soft', ))) {
		$__compilerTemp2 .= '
										<a href="' . $__templater->func('link', array('profile-posts/delete', $__vars['profilePost'], ), true) . '"
											...
											data-xf-click="overlay">' . 'Удалить' . '</a>
										';
		$__vars['hasActionBarMenu'] = true;
		$__compilerTemp2 .= '
									';
	...
	if (($__vars['profilePost']['message_state'] == 'deleted') AND $__templater->method($__vars['profilePost'], 'canUndelete', array())) {
		$__compilerTemp2 .= '
										<a href="' . $__templater->func('link', array('profile-posts/undelete', $__vars['profilePost'], ), true) . '" data-xf-click="overlay"
											class="actionBar-action actionBar-action--undelete actionBar-action--menuItem">' . 'Восстановить' . '</a>
										';
		$__vars['hasActionBarMenu'] = true;
		$__compilerTemp2 .= '
									';
	...
	if ($__templater->method($__vars['profilePost'], 'canWarn', array())) {
		$__compilerTemp2 .= '
										<a href="' . $__templater->func('link', array('profile-posts/warn', $__vars['profilePost'], ), true) . '"
											class="actionBar-action actionBar-action--warn actionBar-action--menuItem">' . 'Предупредить' . '</a>
										';
		$__vars['hasActionBarMenu'] = true;
		$__compilerTemp2 .= '
									';
		...
		$__compilerTemp2 .= '
										<a href="' . $__templater->func('link', array('warnings', array('warning_id' => $__vars['profilePost']['warning_id'], ), ), true) . '"
											...
											data-xf-click="overlay">' . 'Посмотреть предупреждение' . '</a>
										';
		$__vars['hasActionBarMenu'] = true;
		$__compilerTemp2 .= '
									';
	...
	if ($__vars['hasActionBarMenu']) {
		$__compilerTemp2 .= '
										<a class="actionBar-action actionBar-action--menuTrigger"
											data-xf-click="menu"
											title="' . $__templater->filter('Дополнительные параметры', array(array('for_attr', array()),), true) . '"
											role="button"
											tabindex="0"
											aria-expanded="false"
			...
			$__finalCompiled .= '
									<div class="message-responseRow u-jsOnly js-commentLoader">
										<a href="' . $__templater->func('link', array('profile-posts/load-previous', $__vars['profilePost'], array('before' => $__templater->arrayKey($__templater->method($__vars['profilePost']['LatestComments'], 'first', array()), 'comment_date'), ), ), true) . '"
											...
											rel="nofollow">' . 'Посмотреть предыдущие комментарии' . $__vars['xf']['language']['ellipsis'] . '</a>
									</div>
								';
		}
		$__finalCompiled .= '
	...
	if ($__templater->method($__vars['profilePost'], 'canComment', array())) {
		...
		$__finalCompiled .= $__templater->form('
			...
			'minHeight' => '40',
			'placeholder' => 'Напишите комментарий' . $__vars['xf']['language']['ellipsis'],
			'submitText' => 'Отправить комментарий',
			'deferred' => true,
			'simpleSubmit' => true,
		), $__vars) . '
													</div>
													<div class="editorPlaceholder-placeholder">
														<div class="input"><span class="u-muted"> ' . 'Напишите комментарий' . $__vars['xf']['language']['ellipsis'] . '</span></div>
													</div>
												</div>
											</div>
										</div>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<div class="message message--simple' . ($__templater->method($__vars['profilePost'], 'isIgnored', array()) ? ' is-ignored' : '') . ' js-inlineModContainer"
		...
		'log' => $__vars['profilePost']['DeletionLog'],
					...
					<a href="' . $__templater->func('link', array('profile-posts/show', $__vars['profilePost'], ), true) . '" class="u-jsOnly" data-xf-click="inserter" data-replace="[data-content=profile-post-' . $__templater->escape($__vars['profilePost']['profile_post_id']) . ']">' . 'Показать' . $__vars['xf']['language']['ellipsis'] . '</a>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__finalCompiled .= '
			...
			<div class="contentRow-minor">
				...
				<a href="' . $__templater->func('link', array('profile-posts', $__vars['profilePost'], ), true) . '" rel="nofollow" class="contentRow-extra" data-xf-click="overlay" data-xf-init="tooltip" title="' . $__templater->filter('Взаимодействовать', array(array('for_attr', array()),), true) . '">&#8226;&#8226;&#8226;</a>
			</div>
		</div>
	</div>
';
		...
		if ($__vars['comment']['message_state'] == 'moderated') {
			$__finalCompiled .= '
											<a href="' . $__templater->func('link', array('profile-posts/comments/approve', $__vars['comment'], array('t' => $__templater->func('csrf_token', array(), false), ), ), true) . '"
												class="actionBar-action actionBar-action--approve actionBar-action--menuItem">' . 'Одобрить' . '</a>
											';
			$__vars['hasActionBarMenu'] = true;
			$__finalCompiled .= '
										';
			...
			$__finalCompiled .= '
											<a href="' . $__templater->func('link', array('profile-posts/comments/unapprove', $__vars['comment'], array('t' => $__templater->func('csrf_token', array(), false), ), ), true) . '"
												class="actionBar-action actionBar-action--unapprove actionBar-action--menuItem">' . 'Отклонить' . '</a>
											';
			$__vars['hasActionBarMenu'] = true;
			$__finalCompiled .= '
										';
	...
	<div class="message-responseRow">
		...
		'log' => $__vars['comment']['DeletionLog'],
							...
							<a href="' . $__templater->func('link', array('profile-posts/comments/show', $__vars['comment'], ), true) . '" class="u-jsOnly"
								...
								data-replace="[data-content=profile-post-comment-' . $__templater->escape($__vars['comment']['profile_post_comment_id']) . ']">' . 'Показать' . $__vars['xf']['language']['ellipsis'] . '</a>
						</div>
					</div>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	' . $__templater->form('
		...
		'attachmentData' => $__vars['attachmentData'],
		'simple' => true,
		'placeholder' => (($__vars['xf']['visitor']['user_id'] == $__vars['user']['user_id']) ? 'Обновить статус' . $__vars['xf']['language']['ellipsis'] : 'Напишите что-нибудь' . $__vars['xf']['language']['ellipsis']),
		'submitText' => 'Написать',
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	';
	$__vars['placeholder'] = (($__vars['xf']['visitor']['user_id'] == $__vars['user']['user_id']) ? 'Обновить статус' . $__vars['xf']['language']['ellipsis'] : 'Напишите что-нибудь' . $__vars['xf']['language']['ellipsis']);
	$__finalCompiled .= '
	...
	if ($__vars['style'] == 'full') {
		...
		$__compilerTemp1 .= '
			<div>
				<span class="u-srOnly" id="ctrl_message">' . 'Обновить статус' . $__vars['xf']['language']['label_separator'] . '</span>
			...
			'placeholder' => $__vars['placeholder'],
				...
				<div class="u-hidden u-hidden--transition u-inputSpacer">
					' . $__templater->button('Написать', array(
			'type' => 'submit',
