...
class User extends AbstractPlugin
{
	 ...
	 * @api-in bool $username_change_visible If true and the username is changed, the change will be visible (shown in the public list of previous usernames).
	 * @api-in str $password
	 * @api-in int $dob[day]
	...
	public function userSaveProcessAdmin(\XF\Entity\User $user)
	{
		...
		$user->setOption('insert_username_change_visible', $this->filter('username_change_visible', 'bool'));
		...
		if ($dob)
		{
			...
			{
				$profile->setDob($dob['day'], $dob['month'], $dob['year'] ?? 0);
			}
		}
	 ...
	 *
	 * @api-in <req> file $avatar The uploaded new avatar
	 *
	...
	public function actionUpdateAvatar(\XF\Entity\User $user)
	{
		...
		else
		{
			...
			throw $this->exception($this->requiredInputMissing('avatar'));
		}
	}
