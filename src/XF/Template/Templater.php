...
class Templater
{
	...
	public function fnBbCodeSnippet($templater, &$escape, $bbCode, $context, $content, $maxLength, array $options = [], $type = 'html')
	{
		...
		$snippet = $cleaner->render($formatter->wholeWordTrim($bbCode, $maxLength), $parser, $rules);
	...
	{
		$phrase = \XF::phrase($phraseName, $params);
	...
	public function fnPrefixGroup($templater, &$escape, $contentType, $groupId)
	{
		...
		return \XF::phrase(sprintf('%s_prefix_%s.%d', $contentType, 'group', $groupId), [], false);
	}
	...
	protected function getPrefixPhrase($contentType, $prefixId, $phraseType)
	{
		...
		switch ($phraseType)
		{
			...
			case 'help':
				...
				$phrase = \XF::phrase(sprintf('%s_prefix_%s.%d', $contentType, $phraseType, $prefixId), []);
				$phrase->fallback('');
				return $phrase;

			case 'title':
			default:
				return \XF::phrase(sprintf('%s_prefix.%d', $contentType, $prefixId), [], false);
	...
	public function fnUsernameLink($templater, &$escape, $user, $rich = false, $attributes = [])
	{
		...
		$stroke = $this->processAttributeToRaw($attributes, 'stroke');
		...
		if ($stroke)
		{
			$username = "<span class=\"{$usernameStylingClasses} is-stroked\" data-stroke=\"{$username}\"><span>{$username}</span></span>";
		}
		else if ($usernameStylingClasses)
		{
		...
		return "<{$tag}{$hrefAttr} class=\"username {$class}\" dir=\"auto\" itemprop=\"name\" data-user-id=\"{$userId}\"{$xfInitAttr}{$unhandledAttrs}>{$username}</{$tag}>";
	}
	...
	public function formAssetUpload(array $controlOptions)
	{
		...
		$uploadText = $this->filterForAttr($this, \XF::phrase('upload_file'), $null);
